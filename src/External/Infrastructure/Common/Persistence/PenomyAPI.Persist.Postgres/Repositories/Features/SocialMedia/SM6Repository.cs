...
public class SM6Repository : ISM6Repository
{
    ...
    {
        return await _socialGroups.AsNoTracking().AnyAsync(o => o.Id == groupId, ct);
    }

    public async Task<bool> CheckUserJoinedGroupAsync(
        long userId,
        long groupId,
        CancellationToken ct
    )
    {
        return await _socialGroupJoinRequests
            .AsNoTracking()
            .AnyAsync(o => o.CreatedBy == userId && o.GroupId == groupId, cancellationToken: ct);
    }

    public async Task<bool> AddUserJoinRequestByUserIdAndGroupIdAsync(
        long userId,
        long groupId,
        CancellationToken ct
    )
    ...
    public async Task<bool> AddUserToGroupByUserIdAndGroupIdAsync(
        long userId,
        long groupId,
        CancellationToken ct
    )
    {
        await _dbContext
            ...
            .ExecuteAsync(async () =>
            {
                ...
                await _socialGroupsMember.AddAsync(
                    new SocialGroupMember
                    {
                        GroupId = groupId,
                        MemberId = userId,
                        RoleId = 0,
                ...
                await _socialGroups.ExecuteUpdateAsync(
                    o => o.SetProperty(set => set.TotalMembers, o => o.TotalMembers + 1),
                    ct
                );

                await _socialGroupJoinRequests
                    .Where(o => o.CreatedBy == userId && o.GroupId == groupId)
    ...
    public async Task<ICollection<long>> GetGroupManagerByGroupIdAsync(
        long groupId,
        CancellationToken ct
    )
    {
        return await _socialGroupsMember
            .AsNoTracking()
