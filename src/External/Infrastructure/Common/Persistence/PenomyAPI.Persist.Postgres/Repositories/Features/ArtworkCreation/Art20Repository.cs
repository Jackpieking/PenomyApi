...
internal sealed class Art20Repository : IArt20Repository
{
    ...
    public Art20Repository(DbContext dbContext)
    {
        _dbContext = dbContext;
    }

    public Task<bool> IsCurrentCreatorAuthorizedToAccessAsync(
        long creatorId,
        long artworkId,
    ...
    public Task<Artwork> GetDetailToCreateChapterAsync(
        long animeId,
        CancellationToken cancellationToken)
    {
        return _dbContext.Set<Artwork>()
            .AsNoTracking()
            .Where(comic => comic.Id == animeId)
    ...
    public async Task<bool> CreateAnimeChapterAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        await executionStrategy.ExecuteAsync(
            operation: async () =>
                await InternalCreateChapterAsync(
                    animeChapter,
                    chapterVideoMedia,
                    cancellationToken: cancellationToken,
                    result: result
    ...
    private async Task InternalCreateChapterAsync(
        ArtworkChapter animeChapter,
        ArtworkChapterMedia animeChapterMedia,
        CancellationToken cancellationToken,
        Result<bool> result)
    {
        ...
        try
        {
            ...
            await _chapterDbSet.AddAsync(animeChapter, cancellationToken);

            var metadata = new ArtworkChapterMetaData
            {
                ChapterId = animeChapter.Id
            };

            await _chapterMetaDataDbSet.AddAsync(metadata, cancellationToken);

            await _chapterMediaDbSet.AddAsync(animeChapterMedia, cancellationToken);

            // If comic chapter is not upload with drafted mode,
            // then update the upload order of the corresponding anime.
            if (animeChapter.PublishStatus != PublishStatus.Drafted)
            {
                var artworkDbSet = _dbContext.Set<Artwork>();

                await artworkDbSet
                    .Where(artwork => artwork.Id == animeChapter.ArtworkId)
                    .ExecuteUpdateAsync(
