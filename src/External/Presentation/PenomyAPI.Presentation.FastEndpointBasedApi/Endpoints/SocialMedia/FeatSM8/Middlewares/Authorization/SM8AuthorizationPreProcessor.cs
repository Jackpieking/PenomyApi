...
namespace PenomyAPI.Presentation.FastEndpointBasedApi.Endpoints.FeatSM8.Middlewares.Authorization;

internal sealed class SM8AuthorizationPreProcessor : PreProcessor<SM8RequestDto, SM8StateBag>
{
    private readonly TokenValidationParameters _tokenValidationParameters;
    private readonly Lazy<SecurityTokenHandler> _securityTokenHandler;

    public SM8AuthorizationPreProcessor(
    ...
    public override async Task PreProcessAsync(
        IPreProcessorContext<SM8RequestDto> context,
        SM8StateBag state,
    ...
    )
    {
        ...
        if (!validateTokenResult.IsValid || tokenExpireTime <= DateTime.UtcNow)
        {
            await SendResponseAsync(
                SM8ResponseStatusCode.UN_AUTHORIZED,
                state.AppRequest,
                context.HttpContext,
                ct
            );
    ...
    private static Task SendResponseAsync(
        ...
        SM8Request appRequest,
        HttpContext context,
        CancellationToken ct
    )
    {
        var httpResponse = SM8HttpResponseManager
            .Resolve(statusCode)
