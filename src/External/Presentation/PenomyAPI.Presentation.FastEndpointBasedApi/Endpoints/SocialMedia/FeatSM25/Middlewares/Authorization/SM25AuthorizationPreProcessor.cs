...
namespace PenomyAPI.Presentation.FastEndpointBasedApi.Endpoints.FeatSM25.Middlewares.Authorization;

internal sealed class SM25AuthorizationPreProcessor : PreProcessor<SM25RequestDto, SM25StateBag>
{
    private readonly TokenValidationParameters _tokenValidationParameters;
    private readonly Lazy<SecurityTokenHandler> _securityTokenHandler;

    public SM25AuthorizationPreProcessor(
    ...
    public override async Task PreProcessAsync(
        IPreProcessorContext<SM25RequestDto> context,
        SM25StateBag state,
    ...
    )
    {
        ...
        if (!validateTokenResult.IsValid || tokenExpireTime <= DateTime.UtcNow)
        {
            await SendResponseAsync(
                SM25ResponseStatusCode.UNAUTHORIZED,
                state.AppRequest,
                context.HttpContext,
                ct
            );
