...
namespace PenomyAPI.Presentation.FastEndpointBasedApi.Endpoints.FeatSM27.Middlewares.Authorization;

internal sealed class SM27AuthorizationPreProcessor : PreProcessor<SM27RequestDto, SM27StateBag>
{
    private readonly TokenValidationParameters _tokenValidationParameters;
    private readonly Lazy<SecurityTokenHandler> _securityTokenHandler;

    public SM27AuthorizationPreProcessor(
    ...
    public override async Task PreProcessAsync(
        IPreProcessorContext<SM27RequestDto> context,
        SM27StateBag state,
    ...
    )
    {
        ...
        if (!validateTokenResult.IsValid || tokenExpireTime <= DateTime.UtcNow)
        {
            await SendResponseAsync(
                SM27ResponseStatusCode.UNAUTHORIZED,
                state.AppRequest,
                context.HttpContext,
                ct
            );
