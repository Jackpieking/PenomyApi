...
namespace PenomyAPI.Presentation.FastEndpointBasedApi.Endpoints.FeatG52.Middlewares.Authorization;

internal sealed class G52AuthorizationPreProcessor : PreProcessor<G52RequestDto, G52StateBag>
{
    private readonly TokenValidationParameters _tokenValidationParameters;
    private readonly Lazy<SecurityTokenHandler> _securityTokenHandler;

    public G52AuthorizationPreProcessor(
    ...
    public override async Task PreProcessAsync(
        IPreProcessorContext<G52RequestDto> context,
        G52StateBag state,
    ...
    )
    {
        ...
        if (!validateTokenResult.IsValid || tokenExpireTime <= DateTime.UtcNow)
        {
            await SendResponseAsync(
                G52ResponseStatusCode.UN_AUTHORIZED,
                state.AppRequest,
                context.HttpContext,
                ct
            );
